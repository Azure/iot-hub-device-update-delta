...
{
user_exception::user_exception(error_code error) : user_exception(error, "") {}
user_exception::user_exception(error_code error, std::string msg) : m_error(error), m_msg(msg)
{
	populate_callstack();
...
void errors::user_exception::populate_callstack()
{
#ifdef __linux__
	...
	while (true)
	{
		...
		if (static_cast<size_t>(address_count) < stack_addresses.capacity())
		{
			break;
		}
...
errors::user_exception errors::user_exception::value_exceeds_size_t(error_code error, std::string msg, uint64_t value)
{
