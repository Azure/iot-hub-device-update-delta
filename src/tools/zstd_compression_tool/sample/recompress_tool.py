...
def execute(input_archive_path, output_archive_path, zstd_compress_file_path):
    ...
    helpers.create_archive(output_archive_path, working_folder_manager, strings_to_replace=strings_to_replace)

    working_folder_manager.dispose()

def print_usage():
    print(f'{os.path.basename(sys.argv[0])} <input archive path> <output test archive path> <zstd_compress_file path>')
...
def main():
    if len(sys.argv) == 4:
        ...
        execute(input_archive_path, output_archive_path, zstd_compress_file_path)
    else:
        print_usage()
